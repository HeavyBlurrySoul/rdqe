...
app.post('/', (req, res) => {
    ...
    function createTableRow(name, pdssr, pdpsr, pda, pdc, iva, ivc, fc, ft, mt, rng, azim, stat) {
        let tempArr = []
        if (stat === 1) {
             tempArr = [name, req.body.to,pdssr, pdpsr, pda, pdc, iva, ivc, fc, ft, mt, rng, azim, "No Data"]
        } else {
             tempArr = [name, req.body.to,pdssr, pdpsr, pda, pdc, iva, ivc, fc, ft, mt, rng, azim]
    ...
    let missingRadars = allRadars.filter(x => !radarsRunning.includes(x)).concat(radarsRunning.filter(x => !allRadars.includes(x)))
    ...
    resArray.unshift(['Radar', 'Datum', 'Pd_SSR', 'Pd_PSR', 'Pd_A', 'Pd_C', 'Pd_Inc_Val_A', 'Pd_Inc_Val_C', 'Pd_False_Code', 'Rate_False_Trgt', 'Rate_Multi_Trgt', 'Range_Bias', 'Az_Bias', 'Opmerkingen'])
